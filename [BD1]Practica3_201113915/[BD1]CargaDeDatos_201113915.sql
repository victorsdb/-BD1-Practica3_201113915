...
-- ======================================================================================
	INSERT INTO ciudad(nombre_ciudad, pais_ciudad)	
		SELECT DISTINCT ciudad_cliente, id_pais 
			FROM registro
			INNER JOIN pais ON nombre_pais = pais_cliente
...
-- ======================================================================================
	INSERT INTO direccion(direccion, codigo_postal, ciudad_direccion)
		SELECT DISTINCT direccion_cliente, codigo_postal_cliente, id_ciudad
			FROM registro 
				INNER JOIN ciudad ON nombre_ciudad = ciudad_cliente 
				INNER JOIN pais ON id_pais = pais_ciudad and nombre_pais = pais_cliente
...
-- ======================================================================================
	INSERT INTO tienda(nombre_tienda, direccion_tienda)
		SELECT DISTINCT nombre_tienda, id_direccion
			FROM registro 
				INNER JOIN direccion ON direccion.direccion = direccion_tienda
					WHERE nombre_tienda IS NOT null;
-- ======================================================================================
-- Carga de datos de la tabla EMPLEADO
-- ======================================================================================
	INSERT INTO empleado(nombre_empleado, apellido_empleado, correo_empleado, empleado_activo, tienda_empleado, usuario_empleado, contrasenia_empleado, direccion_empleado)
		SELECT DISTINCT nombre_empleado, apellido_empleado, correo_empleado, (CASE WHEN UPPER(empleado_activo)='SI'  THEN true WHEN UPPER(empleado_activo)='NO' THEN false END) as actividad, id_tienda, usuario_empleado, contrasenia_empleado, id_direccion 
			FROM registro 
				INNER JOIN tienda ON tienda.nombre_tienda = tienda_empleado
				INNER JOIN direccion ON direccion.direccion = direccion_empleado
...
-- ======================================================================================
	INSERT INTO encargado_tienda
		SELECT DISTINCT id_tienda, id_empleado
			FROM registro
				INNER JOIN empleado ON empleado.nombre_empleado = nombre_encargado AND empleado.apellido_empleado = apellido_encargado
...
-- ======================================================================================
	INSERT INTO actor(nombre_actor, apellido_actor)
		SELECT DISTINCT nombre_actor, apellido_actor 
			FROM registro 
				WHERE nombre_actor IS NOT null AND apellido_actor IS NOT null
...
-- ======================================================================================
	INSERT INTO pelicula(nombre_pelicula, descripcion_pelicula, anio_lanzamiento, dias_renta, costo_renta, duracion_minutos, costo_por_danio, clasificacion_pelicula)
		SELECT DISTINCT nombre_pelicula, descripcion_pelicula, anio_lanzamiento, dias_renta, costo_renta, duracion_minutos, costo_por_danio, id_clasificacion	 
			FROM registro 
				INNER JOIN clasificacion ON clasificacion.nombre_clasificacion = clasificacion_pelicula
...
-- ======================================================================================
	INSERT INTO actor_pelicula
		SELECT DISTINCT id_pelicula, id_actor
			FROM registro
				INNER JOIN actor ON actor.nombre_actor = registro.nombre_actor AND actor.apellido_actor = registro.apellido_actor
				INNER JOIN pelicula ON pelicula.nombre_pelicula = registro.nombre_pelicula
...
-- ======================================================================================
	INSERT INTO lenguaje_pelicula
		SELECT DISTINCT id_pelicula, id_lenguaje
			FROM registro
				INNER JOIN lenguaje ON lenguaje.nombre_lenguaje = registro.lenguaje_pelicula
				INNER JOIN pelicula ON pelicula.nombre_pelicula = registro.nombre_pelicula
					ORDER BY id_pelicula;
-- ======================================================================================
...
-- ======================================================================================
	INSERT INTO categoria_pelicula
		SELECT DISTINCT id_pelicula, id_categoria
			FROM registro
				INNER JOIN categoria ON categoria.nombre_categoria = registro.categoria_pelicula
				INNER JOIN pelicula ON pelicula.nombre_pelicula = registro.nombre_pelicula
					ORDER BY id_pelicula;
-- ======================================================================================
...
-- ======================================================================================
	INSERT INTO renta_pelicula(cliente_renta, copia_pelicula_renta, empleado_renta, fecha_renta, fecha_retorno)
		SELECT DISTINCT id_cliente, id_copia_pelicula, id_empleado, fecha_renta, fecha_retorno
			FROM registro	
				INNER JOIN cliente ON cliente.nombre_cliente = registro.nombre_cliente AND cliente.apellido_cliente = registro.apellido_cliente
				INNER JOIN pelicula ON pelicula.nombre_pelicula = registro.nombre_pelicula 
				INNER JOIN tienda ON tienda.nombre_tienda = registro.tienda_pelicula
				INNER JOIN empleado ON empleado.nombre_empleado = registro.nombre_empleado AND empleado.apellido_empleado = registro.apellido_empleado
				INNER JOIN copia_pelicula ON copia_pelicula.pelicula = id_pelicula AND copia_pelicula.tienda = id_tienda
	...
	INSERT INTO pago_renta(renta, monto_pago, fecha_pago)
		SELECT DISTINCT id_renta_pelicula, monto_a_pagar, fecha_pago
			FROM registro	
				...
				INNER JOIN empleado ON empleado.nombre_empleado = registro.nombre_empleado AND empleado.apellido_empleado = registro.apellido_empleado
				INNER JOIN copia_pelicula ON copia_pelicula.pelicula = id_pelicula AND copia_pelicula.tienda = id_tienda
				INNER JOIN renta_pelicula ON renta_pelicula.cliente_renta = id_cliente
